...
agents with additive valuations \cite{ChaudhuryGargMehlhorn2024}. It also holds,
for general monotone valuations and any number of agents, whenever the number of
goods is small relative to the number of agents: Mahara \cite{Mahara2024} proved
that an \EFX{} allocation always exists when $\goods\leq\agents+3$. For
$\agents=4$ this already settles all sizes up to $\goods=7$, so the smallest open
four-agent case is $\goods=\agents+4=8$. For four
...
The code, report sources, logs, and retained proof artifacts are available at
\url{https://github.com/MAKMED1337/EFX}.
...
the $(4,8)$ run is computationally demanding: a related three-agent /
seven-good \EFX{} query did not terminate within a $40$-hour budget for the
Z3-over-LRA encoding of \cite{AkramiEtAl2026}, so termination at this scale cannot be
taken for granted. The two encodings are not directly comparable --- theirs is over
\emph{monotone} valuations (a real variable per bundle, with several thousand
constraints), whereas ours is \emph{additive} (the $32$ per-good variables of
\eqref{eq:negation}) --- so their non-termination is suggestive of the difficulty
rather than a prediction for our search space; but both run Z3 over QF\_LRA at a
comparable allocation count, which is enough to make the point.
...
The instance size is supplied on the command line as \texttt{N M}.
...
int N = std::atoi(argv[1]);                    // number of agents
int M = std::atoi(argv[2]);                    // number of goods
...
\EFX{} allocation for every profile. These two sizes satisfy
$\goods\leq\agents+3$ and are therefore already covered by Mahara's analytic
existence theorem \cite{Mahara2024}; we include them as benchmarks --- a check that
the pipeline reproduces known results --- and only $(4,8)=(\agents,\agents+4)$ is
new. Table~\ref{tab:results} summarises the runs.
...
Three lines of work are especially close.

\paragraph{Mahara \cite{Mahara2024}.}
Mahara proved that an \EFX{} allocation always exists when $\goods\leq\agents+3$,
even for general monotone valuations. For $\agents=4$ this covers $\goods\leq 7$,
so the $(4,6)$ and $(4,7)$ rows of Table~\ref{tab:results} were already known
analytically; we run them only as benchmarks. The first size his theorem does not
reach for four agents is exactly $\goods=\agents+4=8$, which is the size we settle
here (for additive valuations).
...
$25$ seconds). They explicitly leave the \emph{additive} case open, and their
...
Mahara \cite{Mahara2024}
  & any & $\leq n{+}3$ & monotone & full \EFX{} exists & proof\\
...
Akrami et al. \cite{AkramiEtAl2026}
  & $\geq3$ & $\geq n{+}5$ & submodular & \emph{counterexample} & SAT\,+\,constr.\\
\textbf{This report}
...
\section{Repository structure and running the code}
\label{sec:repo}
...
\begin{center}
  \url{https://github.com/MAKMED1337/EFX}.
\end{center}
The main files are as follows. \texttt{main.cpp} is the Z3-based prover described
in Section~\ref{sec:impl}; \texttt{common.cpp} enumerates ordered non-empty
partitions; \texttt{run-cpp.sh} compiles and runs a C++ source file;
\texttt{main.py} is an earlier Python/Z3 prototype; \texttt{tex/} contains this
report and bibliography; \texttt{proofs/} contains retained machine-verification
artifacts; \texttt{logs/} contains retained solver logs; and \texttt{experiments/}
contains historical experiment scripts and outputs.

To run the main prover, use
\begin{lstlisting}[language=bash]
./run-cpp.sh main.cpp 4 8
\end{lstlisting}
The two numeric program arguments are $\agents$ and $\goods$: the number of
agents and goods in the fixed instance to test. The current implementation
enumerates ordered partitions into non-empty bundles, so it requires
$\goods\geq\agents$. The program writes the assembled SMT-LIB~2 query to
\texttt{proofs/problem\_\agents\_\goods.smt2} and then calls Z3. A
\texttt{sat} result prints a valuation matrix witnessing a counterexample; an
\texttt{unsat} result means no such counterexample exists for the encoded
instance size; and \texttt{unknown} is inconclusive. With proof generation
enabled at compile time, successful proof extraction writes a corresponding
\texttt{proofs/proof\_\agents\_\goods.z3} file.
...
The prose of this report is licensed under Creative Commons
Attribution-ShareAlike 4.0 International (CC BY-SA 4.0). Source-code listings
included from repository files keep the license stated in those source files.

\lstinputlisting[
  language=C++,
  caption={Full source of \texttt{main.cpp}.},
  label={lst:maincpp-full}
]{../main.cpp}
